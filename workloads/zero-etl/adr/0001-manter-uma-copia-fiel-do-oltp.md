...
O `platform/rds` passou a exportar `db_instance_arn` para que este workload cite
a origem por ARN.
...
os workloads de Redshift antes do `platform/rds`.
...
- `workloads/zero-etl/outputs.tf` — o `next_step`, que é a fronteira honesta do
  Terraform aqui.
- `platform/rds/outputs.tf` — o `db_instance_arn` exportado para isto.
