...
| Requisito | Valor hoje | Origem |
|---|---|---|
| Mecanismo | datashare entre namespaces da mesma conta | `infra/main.tf:236-252` |
| Cópia de dado | **nenhuma** | `infra/main.tf:260-275` |
| Job a operar | **nenhum** | — |
| Código próprio a operar | **0 linhas** | — |
| Escopo do share | todas as tabelas do schema `public` do produtor | `infra/main.tf:243-244` |
| Handshake entre as pontas | dispensado — só existe em cross-account | `infra/main.tf:260-275` |
| Defasagem medida produtor → consumidor | **não medida** (esperada: zero) | — |
| Latência da mesma query nas duas pontas | **não medida** | — |
| Reconciliação do SQL pelo Terraform | **não existe** — roda uma vez, na criação | `infra/main.tf:236-252` |
...
| Requisito | Valor hoje | Origem |
|---|---|---|
| Granularidade da concessão | objeto inteiro (schema, tabela) | `infra/main.tf:243-244` |
...
| Requisito | Valor hoje | Origem |
|---|---|---|
| Workgroups criados | **2** — é o único workload do repositório com dois | `infra/main.tf:108-147` |
| Capacidade base por workgroup | 4 RPU | `infra/variables.tf:33-41` |
| Custo parado | **≠ US$0, dobrado** | `README.md:78-96` |
| Armazenamento duplicado | **nenhum** — o consumidor não guarda nada | `infra/main.tf:260-275` |
