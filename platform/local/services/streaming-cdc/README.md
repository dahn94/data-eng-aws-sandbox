...
```bash
cd platform/local/services/streaming-cdc
cp .env.example .env
...
`../../aws/modules/rds` já configura isso quando o workload liga `enable_logical_replication`; se você estiver usando um Postgres
...
- `platform/local/services/olap-clickhouse/create-table-events.sql`
