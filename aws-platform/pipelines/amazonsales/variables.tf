...
variable "s3_bucket_raw" {
  description = "Bucket S3 com os dados brutos, entrada da pipeline (criado por aws-platform/foundation)"
  type        = string
}

variable "s3_bucket_curated" {
  description = "Bucket S3 onde os resultados de Data Quality são publicados (criado por aws-platform/foundation)"
  type        = string
}
...
variable "lakehouse_arn" {
  description = "ARN do bucket S3 Tables. Vazio = montado a partir da conta, região e ambiente, casando com o nome criado por aws-platform/foundation."
  type        = string
  default     = ""
}
