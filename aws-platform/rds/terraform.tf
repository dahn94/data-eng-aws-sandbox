...
locals {
  ...
  # Configuração extra do backend S3 quando o state remoto está no LocalStack.
  # É um merge de lista possivelmente vazia, e não um condicional simples,
  # porque os dois lados de um `? :` teriam tipos de objeto diferentes e o
  # Terraform recusa ("inconsistent conditional result types").
  localstack_state_config = merge([for _ in range(local.use_localstack ? 1 : 0) : {
    access_key                  = "test"
    secret_key                  = "test"
    endpoints                   = { s3 = var.aws_endpoint_url }
    use_path_style              = true
